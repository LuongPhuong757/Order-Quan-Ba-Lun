 ...
 * có jsdom (xem chú thích đầu `lib/cart-store.test.ts` bên apps/shop). Nên mọi quyết định
 * nằm ở đây dưới dạng hàm thuần, còn boundary chỉ làm hai việc bẩn: đọc mốc thời gian và
 * gọi `location.reload()`.
 *
 * Bản sao y hệt nằm ở `apps/shop/src/lib/chunk-error.ts` — sửa một bên thì sửa cả hai.
 *
