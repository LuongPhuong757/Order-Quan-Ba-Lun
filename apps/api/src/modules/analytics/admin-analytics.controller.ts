...
export class AdminAnalyticsController {
  ...
  async traffic(@Query('days') days?: string) {
    ...
    // Chạy song song: 6 câu độc lập nhau, tuần tự thì cộng dồn latency vô ích.
    const [totals, by_day, by_hour, paths, active, geo_share, geo_failures] = await Promise.all([
      ...
      // Chi tiết từng lần hỏng (2026-09-04) — cùng lý do đi kèm `traffic` như dòng trên.
      geoShareFailures(this.ds, range),
    ]);
    return {
      data: {
