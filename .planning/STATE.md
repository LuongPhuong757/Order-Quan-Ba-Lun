...
- **NỢ FONT (ảnh hưởng phase 8):** `tokens.css` khai báo `--font-display: 'Baloo 2'` và `--font-body: 'Be Vietnam Pro'` nhưng **chưa self-host** — chưa có `apps/shop/public/fonts/` lẫn `fonts.css`. Hiện cả trang rơi về font hệ thống (Segoe UI trên Windows). Wordmark vì thế chưa có nét bo tròn ấm như thiết kế. Phải tải `.woff2` (subset latin + vietnamese) vào `public/fonts/` ở phase 8 — tokens.css ghi rõ KHÔNG gọi Google Fonts CDN (khách mạng 3G VN).
