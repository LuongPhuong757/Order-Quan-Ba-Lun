...
| **M4.D-36** | **Quyển menu hiện ĐÚNG danh sách POS.** Món nào nhân viên gọi được ở màn gọi món thì khách thấy ở `/thuc-don`; sắp nhóm theo `sort_order`, món theo tên — y như POS. **Gỡ hẳn màn "Menu xem"** cùng đường ghi `is_menu_hidden` (món + nhóm), `menu_sort_order`, `PUT /menu/book-order`. Cột DB giữ lại, không đọc. | ⚠ **Chủ quán chốt 2026-09-15.** Khách quét QR không thấy món nhân viên vẫn bán được thì khách đi hỏi nhân viên — đúng việc M4 muốn giảm. Một nguồn sự thật thì không có ca hai màn nói hai chuyện. Hệ quả chấp nhận: nhóm không phải đồ ăn (cược tiền mang về, ứng tiền, mục ngoài quán, thuốc lá) cũng hiện và cộng được vào giỏ — nhân viên soát ở preview (M4.D-17). Đây cũng là cách đóng lỗ hở "sinh mã được cho món đã ẩn khỏi quyển menu": không còn khái niệm ẩn nữa. |
| **M4.D-37** | **Giỏ QR đổ thẳng vào MÀN GỌI MÓN đã có (`BulkOrderModal`), không còn màn preview riêng.** Gõ mã → món của khách nằm sẵn trong giỏ → nhân viên sửa SL / bỏ món / **gọi thêm** ngay tại đó → bấm **Báo bếp** một lần, món xuống bếp luôn (`send_to_kitchen: true`). `…/apply` nay nhận `items` (danh sách nhân viên chốt) thay vì chỉ `skip_menu_item_ids`. **ĐẢO NGƯỢC M4.D-17 và M4.D-18.** | ⚠ **Chủ quán chốt 2026-09-16, nguyên văn: "tại sao lại cứ tạo thêm màn mới trong khi có thể dùng luôn màn gọi món cũ và fill hết số món từ mã vào".** Màn preview cũ chỉ bỏ được dòng — không sửa được số lượng, không gọi thêm được — nên khách đổi ý một câu là phải đi qua thêm một màn nữa. Và món vào ở `PENDING` khiến bàn 8 món phải bấm "Báo bếp" 8 lần (không có nút gửi cả lượt). Nội dung preview KHÔNG mất: mã, giờ sinh mã, số phần, tổng tiền, cảnh báo lệch giá / hết món nay là **dải đầu màn gọi món** — cùng chốt chặn "giỏ của bàn khác" (M4.D-01), nhưng nằm ngay trên đống món đang được nhìn. **Việc chiếm mã không đổi**: nút Báo bếp vẫn đi qua `…/apply`, vẫn compare-and-set `used_at IS NULL` (M4.D-13). BE vẫn tự bỏ món hết hàng dù FE gửi gì, và vẫn tra giá từ menu — FE không đặt được giá. |
| **M4.D-38** | Thêm nút **📢 Báo bếp tất cả** ở đầu vùng "Đang gọi" trong drawer bàn — gửi cả lượt qua `POST /orders/:id/send-to-kitchen` (endpoint đã có sẵn từ đầu, trước nay chưa màn nào gọi tới). Nút "Báo bếp" trên từng món **giữ nguyên**. | Sau M4.D-37 thì vùng "Đang gọi" gần như luôn rỗng, nhưng vẫn còn đường khác rơi vào đó (gọi thêm cho nhóm chưa qua bếp, hoặc `apply` với `send_to_kitchen: false`). Có nút này thì không bao giờ phải bấm từng món nữa. Nút lẻ giữ lại vì đẩy một phần xuống trước (nước ra ngay, đồ nướng để sau) là thao tác thật. |
| **M4.D-39** | **Sau khi nhân viên nhận mã, màn của khách ẩn 5 chữ số và hiện DANH SÁCH MÓN CỦA BÀN** kèm tổng tiền, tự làm tươi mỗi 5 giây. Là món của cả BÀN (gồm lượt gọi trước + món nhân viên gõ thêm), gộp theo tên **và giá**, bỏ dòng đã huỷ và dòng ghi chú cho bếp. `GET /api/public/dine-in-carts/:code` trả thêm `table_items` + `table_subtotal`, **chỉ khi** mã đã `USED` **và** người hỏi gửi đúng `customer_token` (header `X-Customer-Token`). Mã đã nhận nhớ ở khoá riêng `qbl.dinein.accepted.v1`, TTL 4 giờ. | ⚠ **Chủ quán chốt 2026-09-16.** Trước đó màn chỉ nói "Nhân viên đã nhận món của bạn" — khách không soát được gì, và vẫn phải hỏi nhân viên đã gọi những gì. Chọn "món của BÀN" thay vì "món của mã" để khách thấy luôn món gọi thêm bằng miệng và thấy đúng thứ nhân viên đã sửa. **`customer_token` là bắt buộc, không phải tuỳ chọn**: mã chỉ 5 chữ số, dò hết là chuyện vài giây — không có nó thì ngồi một chỗ dò mã là đọc được cả quán đang ăn gì. Vẫn KHÔNG kèm mã bàn / tên bàn / tên nhân viên (hard gate cũ giữ nguyên), và KHÔNG kèm trạng thái bếp của từng món: khách nhìn "đang nấu" rồi đếm phút là sinh ra đúng loại câu hỏi M4 muốn giảm. Khoá nhớ phải TÁCH khỏi `qbl.dinein.code.v1` vì khoá đó tự bỏ mã sau 15 phút, mà bữa ăn dài hơn thế. |
| **M4.D-40** | **Gỡ hẳn hạn mức sinh mã theo thiết bị. ĐẢO NGƯỢC M4.D-26.** Không còn trần 5 mã/giờ, không còn `countRecentByToken`, không còn lỗi `TOO_MANY_REQUESTS` ở luồng này. Chặn spam chỉ còn ở tầng IP: `@Throttle` 10 req/phút ở controller + throttler toàn cục 600 req/phút. | ⚠ **Chủ quán chốt 2026-09-16 sau khi gặp thật: "Bạn đã tạo quá nhiều mã trong một giờ. Vui lòng gọi nhân viên hỗ trợ."** Con số 5 chật hơn một bữa ăn: bàn gọi ba lượt, thêm vài lần bấm "Sửa lại món" (mỗi lần huỷ mã cũ + sinh mã mới, và **mã đã huỷ vẫn bị đếm**) là chạm trần giữa bữa. Khi đó câu báo bảo khách đi gọi nhân viên — tức là mất trắng mục tiêu của cả milestone. Cái mất đi nhỏ: giỏ sinh ra là dòng TRƠ, không vào bill, không tới bếp, không ai thấy, hạn 15 phút, cho tới khi một nhân viên ĐÃ ĐĂNG NHẬP gõ đúng 5 số vào một bàn đang mở. Index `idx_dic_token_created` giữ lại để truy vết. Nếu sau này cần siết theo thiết bị thì đặt ngưỡng theo BỮA ĂN (vd 30/giờ), đừng quay lại con số 5. |
