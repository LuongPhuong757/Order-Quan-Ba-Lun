...
# Generated by /vg:project Round 7 via vg_generate_config.py.
# Edit freely — workflow re-reads every command; no regeneration needed unless foundation changes.

# === Project Identity ===
project_name: "OrderQuanBaLun"
project_description: "Web app quản lý order món ăn cho quán nhỏ — POS tại quán + takeaway + delivery"
...
environments:
  local:
    ...
    test_runner: "npx vitest run"    # e.g., npx vitest run | pytest | cargo test
    dev_command: "pnpm dev"  # hotreload dev server
  ...
  sandbox:
    ...
    shell: "bash"
    run_prefix: "ssh TBD"
    project_path: "/home/deploy/orderquanbalun"
    deploy:
      pre: "git push origin main"
      build: "git pull origin main && pnpm install && pnpm run build"
      restart: "pm2 reload orderquanbalun-api --update-env"     # e.g., pm2 reload app --update-env
      health: "curl -sf http://localhost:3000/health"
      rollback: "pm2 stop all && git checkout {prev_sha} && pnpm install && pnpm run build && pm2 reload all --update-env"
...
models:
  ...
  contract_gen: "sonnet"           # blueprint 2b — follow format template
  test_goals: "sonnet"           # blueprint 2b5 — convert decisions to goals
  executor: "sonnet"           # build step 8 — copy contracts + follow rules
  debugger: "opus"           # build step 9 — post-wave failure needs reasoning
  scanner: "haiku"           # review phase 1/2 — parallel scanners
...
# === Deploy Profile ===
deploy_profile: "pm2"   # pm2 | docker | systemd | git_push | custom
deploy_profiles:
  pm2:
...
services:
  local:
    - name: "API"
      ...
      required: true
    - name: "Mysql"
      check: "mysqladmin ping -h localhost 2>/dev/null"
      required: true
    - name: "Memory"
      check: "true"
      required: true
...
critical_domains: "ordering,payment,inventory"
...
contract_format:
  ...
  compile_cmd: "pnpm exec tsc --noEmit"   # run after extract to validate contract syntax
...
infra_deps:
  ...
  services:
    mysql:
      check_local: "mysqladmin ping -h localhost 2>/dev/null"
      check_sandbox: "mysqladmin ping -h localhost 2>/dev/null"
      label: "Mysql"
    no_ui_e2e:
      check_local: "false"
      check_sandbox: "false"
